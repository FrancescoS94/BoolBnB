...
class MessagesChartDataController extends Controller
{
    ...
    ////////// FZ PER MOSTRARE LE STATISTICHE DEI MESSAGGI RICEVUTI DA UN DETERMINATO UTENTE PER QUEL DETERMINATO APPARTAMENTO
    ...
    function getMonthlyMessagesData(){
        ...
        $messages_dates = Message::where('flat_id', '=', $flatId)->orderBy('created_at','ASC')->pluck('created_at');
        ...
        // >>>> 2 - ottenere il numero di messaggi ricevuti per ciascun mese per questo appartamento (getMonthlyMessagesData)
        ...
        if(!empty($month_array)){                               // SE l'array dei mesi non è vuoto
            foreach($month_array as $month_number => $month_name){  // lo cicliamo usando il num dei mesi come key e il nome dei mesi come value
                // >>>> 3 - ottenere il numero di messaggi ricevuti in un mese per questo appartamento e inserirli in un array (getMonthlyMessagesCount)
                $monthly_messages_count = Message::whereMonth('created_at', $month_number)->where('flat_id', '=', $flatId)->get()->count();     // ottengo il numero dei messaggi ricevuti in quel mese
                array_push($monthly_messages_count_array, $monthly_messages_count);         // riempio i due array vuoti creati in precedenza
                array_push($month_name_array, $month_name);
            }
        }
        
        // calcolo il numero massimo di messaggi
    ...
    // function getMonthlyMessagesData(){
    //     // $flatId = $_GET['flatId'];
    //     // dd($flatId, 'getMonthlyMessagesData');
